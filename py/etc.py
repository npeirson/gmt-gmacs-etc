...
class simulate:
	...
	def atmoext(self,atmo_ext=self.atmo_ext):
		try:
	...
	def skyfile(self,_moon_day=self.moon_day):
		try:
			if (int(_moon_day) == 0):
	...
	def mirrors(self,_telescope_mode=self.telescope_mode):
		try:
			...
			self.area = edl.area[1] * (u.meter**2) # defaults to all theoretical mirrors


	# grating opt
	def grating(self,_grating_opt=self.grating_opt):
		try:
			...
			self.delta_lambda_default = edl.dld[0] * (u.meter**2) # defaults to low resolution
	...
	def resample_dichroic(self,new_wavelength=self.wavelength):
		try:
	...
	def resample_grating(self,new_wavelength=self.wavelength):
		try:
	...
	def resample_ccd(self,new_wavelength=self.wavelength):
		try:
	...
	def session(self):
		...
		self.counts_noise = (self.sky_flux * extension * self.area) # number of photons. should be unitless
	...
	def gen_err(self):
		...
		self.obs_sky_background = {self.red_noise,self.blue_noise}
