...
from astropy import astropyconst13 as const
...
def fun_callback(galaxy_sb1=galaxy_sb1,galaxy_sb2=galaxy_sb2,galaxy_sb3=galaxy_sb3,galaxy_sb4=galaxy_sb4,galaxy_sb5=galaxy_sb5,galaxy_sb6=galaxy_sb6,galaxy_s0=galaxy_s0,galaxy_sa=galaxy_sa,
				...
				cds_os_nonoise_red=cds_os_noise_red,cds_os_nonoise_blue=cds_os_nonoise_blue,cds_wavelength=cds_wavelength,cds_noise=cds_noise,cds_signal=cds_signal,
				spect_res=spect_res,
	...
	if (cur_tab >= 0) && (cur_tab <= 2):
		...
		if widget_object_types.active is not None:
			if widget_object_types.active is 0:
				...
				if widget_types_types.value is not None:
		...
		if widget_types_types.active is not None:
	...
	#cds_snr_red
		if (cb_obj.name == widget_slit_width.name) or (cb_obj.name == widget_binned_pixel_scale.name) or (cb_obj.name == widget_grating_types.name) or (cb_obj.name == widget_seeing.name):
			...
			bps = widget_binned_pixel_scale.active + 1 # because zero-index
			delta_lambda_default = delta_lambda[widget_grating_types.active]
			if (bps == 0):
				bp_const = 12
			elif (bps == 2):
				bp_const = 4
		...
		if (cb_obj.name == widget_wavelengths.name):
			new_spec_wavs=cds_wavelength.data['wavelength']
			old_spec_wavs=cds_snr_red.data['xr']
			spec_fluxes=cds_snr_red.data['yr'], spec_errs=None
		...
		#b =  array.slice().map( function(row){ return row.slice(); });
			...
			for j in range(new_spec_wavs.shape[0]):
				...
				if stop == start:
					...
					spec_widths[stop] /= end_factor
			if spec_errs is not None:
				spec_fluxes = resampled_fluxes
				spec_fluxes_errors = resampled_fluxes_errs
			else: 
				spec_fluxes = resampled_fluxes
				print(spec_fluxes)
		# signal/sqrt(signal+noise+RN^2)
		#TODO: replace 0.1, use variable global_plot_step or something
			print('posting data')
			cds_snr_red.data['xr'] = [widget_wavelengths.value[0]]+[(widget_wavelengths.value[0]+(0.1*i)) for i in range(int((widget_wavelengths.value[1]-widget_wavelengths.value[0])/0.1))]+[widget_wavelengths.value[1]]
			cds_snr_red.data['yr'] = _gal_file / sqrt(cds_signal.data['signal'] + cds_noise.data['noise'] + (rn * rn))
			cds_snr_red.change.emit()
...
widget_grating_types.callback = coalesced_callback
widget_binned_pixel_scale.callback = coalesced_callback
...
widget_wavelengths.callback = coalesced_callback
